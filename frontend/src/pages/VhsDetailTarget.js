...
import { vhsAPI, reviewAPI, rentalAPI } from '../services/api';
...
const VhsDetail = () => {
  ...
  const { user } = useAuth();
  const [vhs, setVhs] = useState(null);
  ...
  const loadVhsDetail = async () => {
    try {
      const response = await vhsAPI.getById(id);
      setVhs(response.data);
    } catch (error) {
  ...
  const loadReviews = async () => {
    try {
      const response = await reviewAPI.getVhsReviews(id);
  ...
  if (!vhs) return <div >VHS not found</div>;
  ...
  const handleRent = async () => {
    ...
    try {
      await rentalAPI.rent({ vhsId: parseInt(id), rentalDays });
      setMessage('VHS rented successfully!');
      loadVhsDetail();
      setTimeout(() => setMessage(''), 3000);
    } catch (error) {
      setMessage(error.response?.data?.message || 'Failed to rent VHS');
    }
  };
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      await reviewAPI.create({ vhsId: parseInt(id), ...reviewForm });
